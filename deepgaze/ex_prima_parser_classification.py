...
def create_csv(input_path, output_path, img_size=64, colour=True, normalisation=False):
    ...
    for folder in os.walk(input_path + "/"):
        ...
        for image_path in glob.glob(str(folder[0]) + "/*.jpg"):
            ...
            if( (pan >= -90) and (pan <= -50)) : label_pan = "Full right"
            elif((pan >= -51) and (pan <= -10)): label_pan = "Slightly right"
            elif((pan >= -11) and (pan <= 10)): label_pan = "Center"
            elif((pan >= 11) and (pan <= 50)): label_pan = "Slightly left"
            elif((pan >= 51) and (pan <= 90)): label_pan = "Full left"
            else: raise ValueError('ERROR: The pan is out of range ... ' + str(pan))

            #Write the CSV file for tilt
            if((tilt >= -90) and (tilt <= -31)): label_tilt = "Full down"
            elif((tilt >= -30) and (tilt <= -11)): label_tilt = "Slightly down"
            elif((tilt >= -10) and (tilt <= 10)): label_tilt = "Center"
            elif((tilt >= 11) and (tilt <= 30)): label_tilt = "Slightly up"
            elif((tilt >= 31) and (tilt <= 90)): label_tilt = "Full up"
...
def main():
    ...
    create_loo_pickle(csv_path="../HeadPoseImageDatabase/training/prima_label.csv", output_path="../HeadPoseImageDatabase/training", shuffle=False)
    ...
    #show_pickle_element(pickle_file="../HeadPoseImageDatabase/training/prima_p1_out.pickle", element=element, element_type="training", img_size=64)
